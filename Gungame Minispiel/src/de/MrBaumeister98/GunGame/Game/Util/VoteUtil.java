...
public class VoteUtil {
	...
	private static void castVote(Arena arena, String map) {
		if(arena.voteMap.get(map) != null) {
			Integer votes = arena.voteMap.get(map);
			votes++;
			arena.voteMap.put(map, votes);
		} else {
			arena.voteMap.put(map, 1);
		}
		Debugger.logInfoWithColoredText(ChatColor.YELLOW + "Arena: " + ChatColor.RED + arena.getName() + ChatColor.YELLOW + ": Voting: Map: " + ChatColor.AQUA + map + ChatColor.YELLOW + " has " + ChatColor.LIGHT_PURPLE + arena.voteMap.get(map).toString() + ChatColor.YELLOW + " Votes!");
	}
	...
	public static ArenaWorld chooseVictorMap(Arena arena) {
		...
		for(String map : arena.voteMap.keySet()) {
			Debugger.logInfoWithColoredText(ChatColor.YELLOW + "    - Map: " + ChatColor.AQUA + map + ChatColor.YELLOW + " has " + ChatColor.LIGHT_PURPLE + arena.voteMap.get(map).toString() + ChatColor.YELLOW + " Votes!") ;
		}
		Integer votes = 0;
		List<String> test = new ArrayList<String>();
		for(String gameMap : arena.voteMap.keySet()) {
			test.add(gameMap);
			int votesOfMap = arena.voteMap.get(gameMap);
			if(votesOfMap >= votes) {
				votes = votesOfMap;
				if(gameMap != "???") {
		...
		if(maps.isEmpty() == false) {
			for(int i = 0; i < 4; i++) {
				if(maps.size() >= 1) {
					...
					arena.voteMap.put(map.getName(), 0);
					maps.remove(map);
				}				
			}
			arena.voteMap.put("???", 0);
	...
	private static ArenaWorld getRandomMap(Arena arena) {
		...
		for(String s : arena.voteMap.keySet()) {
			mapNames.add(s);
		}
		mapNames.remove("???");
